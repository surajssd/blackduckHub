...
_TAG=$(find . -name "appmgr.hubinstall*.zip" | sed 's/.*full-//' | sed 's/\.zip//' | cut -d "/" -f 2)
if [ "$_TAG" == "" ]; then
...
# this is needed so that blckdck user can write files in that dir
sudo chmod 777 -R appmgr.hubinstall-*
